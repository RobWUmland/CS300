...
void LinkedList::Append(Bid bid) {
    Node* newNode = new Node(bid); //Create new node
...
void LinkedList::Prepend(Bid bid) {
    ...
    if (head == nullptr) { //if there is nothing at the head...
        head = newNode; // new node points to current head as its next node
        tail = newNode;
    }
    else {
...
void LinkedList::Remove(string bidId) {   
    ...
    if (head->bid.bidId == bidId) { // special case if matching node is the head
        Node* temp = head;
    ...
    while (current->next != nullptr && current->next->bid.bidId != bidId) { // while loop over each node looking for a match
        current = current->next;// current node is equal to next node
    }

    if (current->next != nullptr) { // if the next node bidID is equal to the current bidID
...
Bid LinkedList::Search(string bidId) {    
    ...
    while (current != nullptr) { // keep searching until end reached with while loop (current != nullptr)
        if (current->bid.bidId == bidId) { // if the current node matches, return current bid
            return current->bid;
        }
        else current = current->next; // else current node is equal to next node
    }
    //(the next two statements will only execute if search item is not found)
    Bid emptyBid; //create new empty bid
